...
export const SellComponent = () => {
	...
	useEffect(() => {
		if (result) {
			setIdrValue(result?.amount_in_currency?.toFixed(2))
		} else {
			setIdrValue('0')
		}
					...
					<div className='border border-[#FFFFFF4D] px-[12px] rounded-[11px] h-[56px] flex justify-between items-center mt-[15px]'>
						<p className='font-normal'>
							...
							<span className='the-bold'>
								{idrValue} {currentSelectedCurrency?.currency}
							</span>
						</p>
						<ArrowDown className='fill-current h-4 ' />
					</div>
						...
						onClick={() => {
							...
							dispatch(
								setCurrentSelectedOfframpCoin({
									...
									currency: currentSelectedCurrency?.currency,
									id: currentSelectedCoin?.id,
									country: currentSelectedCurrency?.value,
								})
							)
							router.push({
								pathname: '/login',
